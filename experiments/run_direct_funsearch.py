...
from llm.code_generator import build_code_generator
from llm.sandbox import load_priority_function
from search.direct_funsearch import DirectFunSearch
...
DEFAULT_LOG_DIR = ROOT / 'results' / 'logs' / 'direct'
DEFAULT_OUTPUT = ROOT / 'results' / 'tables' / 'direct_funsearch_best.csv'
DEFAULT_TEST_OUTPUT = ROOT / 'results' / 'tables' / 'direct_funsearch_test.csv'
...
def main() -> None:
    ...
    engine = DirectFunSearch(population_size=8, generator=generator)
    ...
    if best is None:
        raise SystemExit('No valid direct FunSearch candidates were produced.')

    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open('w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['method', 'score', 'code', 'metrics'])
        writer.writeheader()
        writer.writerow({'method': best.method, 'score': best.score, 'code': best.code, 'metrics': best.metrics})

    if test_instances:
        test_summary = evaluate_priority_function(
            method_name='direct_funsearch_best',
    ...
    print(f'Saved best direct FunSearch candidate to {output}')
